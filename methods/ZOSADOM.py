...
class ZOSADOM(BaseDecentralizedMethod):
    def __init__(self, oracles, topology, max_iter, wandbrun: Run):
        super().__init__(oracles, topology, wandbrun)
        self.max_iter = max_iter
        self.x_all: list[list[Tensor]] = [
    ...
    def __calculate_params(self):
        L = (
            np.sqrt(self.oracles[0].dimension)
            * self.oracles[0].lipschitz
            / self.oracles[0].gamma
        )
        ...
        self.params["beta"] = 1.0 / (2 * L)
        self.params["nu"] = mu / 2
        self.params["alpha"] = mu / 4
        self.params["tau2"] = np.sqrt(mu / L)
        self.params["pi"] = self.params["beta"] / 16
        self.params["tau1"] = 1.0 / (1.0 / self.params["tau2"] + 0.5)
        self.params["eta"] = 1.0 / (
            (1.0 / self.params["beta"] + L) * self.params["tau2"]
        )
        self.params["sigma2"] = np.sqrt(self.params["beta"] * mu) / (16 * chi)
        self.params["sigma1"] = 1.0 / (1 / self.params["sigma2"] + 0.5)
        self.params["kappa"] = self.params["nu"] / (
            14 * self.params["sigma2"] * chi * chi
        )
    ...
    def step(self):
        assert self.topology.matrix_type.startswith(
            "gossip"
        ), "works with gossip matrices only yet"
        gossip_matrix: Tensor = torch.tensor(
            next(self.topology), dtype=self.x_all[0][0].dtype
        )
        layers_count = len(self.x_all[0])
        oracles_counter = range(len(self.oracles))
        updated_params_by_layer = {
            "x": [],
            "y": [],
            "z": [],
            "m": [],
        ...
        for layer_num in range(layers_count):
            x: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.x_all]
            )
            y: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.y_all]
            )
            z: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.z_all]
            )
            x_f: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.x_f_all]
            )
            y_f: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.y_f_all]
            )
            z_f: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.z_f_all]
            )
            m: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.m_all]
            )
            grad: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in grad_all]
            )
            ...
            x_new = (
                ...
                - self.params["nu"] * x_g
                - y
                + self.params["teta"] / self.params["nu"] * (y_g + z_g)
            )
            x_new *= self.params["eta"] / (
    ...
    def run(self, log: bool = False, disable_tqdm=False):
