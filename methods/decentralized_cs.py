...
class DecentralizedCommunicationSliding(BaseDecentralizedMethod):
    ...
    def __calculate_params(self, k=1):
        ...
        self.params = {}
        self.params['mu'] = mu
        self.params['eta'] = k * mu / 2
        self.params['T'] = 100
        self.params['alpha'] = k / (k + 1)
        self.params['teta'] = k + 1
        self.params['tau'] = 4 * self.topology.norm ** 2 / ((k + 1) * mu)
    ...
    def step(self, k = 0):
        ...
        gossip_matrix: Tensor = torch.tensor(next(self.topology), dtype=self.x_all[0][0].dtype) * self.topology.lambda_max
        ...
        x, x_hat = self.communication_sliding(self.x_all, self.change_dim_layer_to_oracle(updated_params_by_layer["w"]))
        self.x_hat = x_hat
        self.x_all = x 
