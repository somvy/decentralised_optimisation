...
class DecentralizedCommunicationSliding(BaseDecentralizedMethod):
    ...
    def communication_sliding(self, x, w):
        ...
        for t in range(self.params["T"]):
            ...
            for o in range(oracles_count):
                ...
                for l in range(layers_count):
                    self.params["beta"] = (
                        t / 2 + (t + 2) / 2 * self.params["mu"] / self.params["eta"]
                    )
        ...
        for o in range(oracles_count):
            for l in range(layers_count):
                u_hat[o][l] /= (
                    self.params["T"] * (self.params["T"] - 1) / 2 + self.params["T"]
                )
        return u, u_hat
    ...
    def step(self, k=0):
        ...
        gossip_matrix: Tensor = (
            torch.tensor(next(self.topology), dtype=self.x_all[0][0].dtype)
            * self.topology.lambda_max
        )
        ...
        for layer_num in range(layers_count):
            ...
            x_prev: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.x_prev]
            )
            x_hat: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.x_hat]
            )
            y: Tensor = torch.stack(
                [oracle_params[layer_num] for oracle_params in self.y]
            )

            x_wave = self.params["alpha"] * (x_hat - x_prev) + x
            n, *d = x_wave.shape
            v = torch.matmul(gossip_matrix, x_wave.view(n, -1)).view(n, *d)
            y = y + 1.0 / self.params["tau"] * v
            n, *d = y.shape
        ...
        x, x_hat = self.communication_sliding(
            self.x_all, self.change_dim_layer_to_oracle(updated_params_by_layer["w"])
        )
        self.x_hat = x_hat
        self.x_all = x
    ...
    def run(self, log: bool = False, disable_tqdm: bool = False):
        if log:
            self.logs.append(self.log())
        for i in trange(self.max_iter, disable=disable_tqdm):
